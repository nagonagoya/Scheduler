...
//◆指定したd番目の日付の日程決定状態に応じたクラス名を返す
function mc_decision(d){
		return d==decision ? "decision1" : "decision0";
}

//◆決定状態に応じたクラス名を「決定」タイトルに返す
function mc_decision_title(){
	return decision==999 ? "decision_title0" : "decision_title1";
}
...
//テーブルのヘッダー作成１
//案内文
var str_head ="<tr><td colspan=\""+(persn.length+5)+"\">案内文</td></tr>"
str_head +="<tr><td class=\"r0\" colspan=\"2\"></td>";
str_head +="<td colspan=\""+(persn.length+3)+"\" class=\"r0\">";
str_head +="<textarea id=\"event_text\" class=\"togglebtn0\" cols=\"50\" rows=\"5\" style=\"width:500px; text-align:left;\">"+text+"</textarea>";
str_head +="</td></tr>";

//テーブルのヘッダー作成２
//決定＆ランキング
str_head +="<tr><td class=\""+mc_decision_title()+"\" colspan=\"2\">決定</td>";
//開始月
str_head +="<td colspan=\"2\" class=\"youbiD\">"+month+"月</td>";
//招待者名
for(let i = 0; i < persn.length; i++){
	//■とりあえず今は招待者のマークなどは触れないことにする
	//str_head +="<td id=\"p"+("0"+i).substr(-2)+"\" class=\""+mc_person(i)+"\">"+persn[i]+"</td>";
	str_head +="<td id=\"p"+("0"+i).substr(-2)+"\" class=\"person_other\">"+persn[i]+"</td>";
}
//コメント
...
for(let d = 0; d < days.length; d++){
	str_body +="<tr>";
	//決定
	str_body +="<td  id=\"decision"+(100+d)+"\" class=\""+mc_decision(d)+"\">"+(d==decision ? "◎" : "　")+"</td>";
	//ランキング
	...
	for(let p = 0; p < persn.length; p++){
		...
		//■とりあえず今は管理者はマークは触れないことにする
		str_body +="<td id="+(100*p+d)+" class=\"fB"+marks[p][d]+"\" title=\""+notes[p][d]+"\">"+mark[marks[p][d]]+"</td>";
...
$("#table1").append(str_body);

//テーブルのフッタの作成
//ランキング
var str_foot ="<td class=\"r0\" colspan=\"2\"></td>";
//登録ボタン
str_foot +="<td id=\"save\" colspan=\""+(persn.length+3)+"\" class=\"btn_entry\">登　　録</td>";
$("#table1").append(str_foot);
...
//■案内文変更時
$('#event_text').focusout(function(e) {
	var id  = $(this).attr("id");	// id名の取得
	var str = $(this).val();
	console.log("evet_text: focusout :"+str);
...
$(".fC0, .fC1, .fC2, .fC3").click(function() {
	...
	//■ランキングをチェックし表示する
	ranking = set_ranking();
	//console.log("ranking2: "+ranking);
	disp_ranking();
});

//■decisionテーブルをクリックされた時の処理：decisionの変更 ＆ class名の変更 ＆ textの変更
$(".decision0, .decision1").click(function() {
	var id  = $(this).attr("id");						// idの数値部分の取得
	var d   = (id.substr(8))%100;									// 日付
	console.log("decision:pressed,  id: "+id+",  d: "+decision+", → "+d);
	if(d==decision){
		$(this              ).attr('class', "decision0").text("　");	// 決定日のClassと記号を更新
		decision = 999;
		$(".decision_title1").attr('class', "decision_title0").text("未決定");
	}else{
		$(".decision1"      ).attr('class', "decision0").text("　");	// 旧決定日のClassと記号を更新
		$(this              ).attr('class', "decision1").text("◎");	// 新決定日のClassと記号を更新
		$(".decision_title0").attr('class', "decision_title1").text("決定");
		decision = d;
	}
});
...
$('#save').click(function() {
	...
	$.get('s_data_exists.php',{filename:"data/data" + id + thread},function(rt){
		if(rt=="false"){
